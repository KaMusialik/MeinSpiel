...
class KA_rentenCSV():
    ...
    def LeseRentenCSV(self, key_dict):
        ...
        if df1.empty:
            wert = 0
            text = 'KA_rentenCSV/LeseRentenSaCSV: Eintrag in der Tabelle fuer Renten nicht gefunden. Es wurde null verwendet: nr='+str(nr)+' von='+str(von)+' bis='+str(bis)+' name='+str(name)
            print(text)
        else:
            index=df1.index[0]
